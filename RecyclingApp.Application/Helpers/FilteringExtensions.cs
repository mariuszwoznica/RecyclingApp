...
namespace RecyclingApp.Application.Helpers
{
    ...
    {
        public static IQueryable<T> ApplayPaging<T>(this IQueryable<T> query, int page, int limit)
        {
            return query
                .Skip((page - 1) * limit)
                .Take(limit);
        ...
        public static IQueryable<T> ApplaySorting<T>(this IQueryable<T> query, string sortingParams)
        {
            if (sortingParams != null)
            {
                ...
                foreach (var param in paramiters)
                {
                    ...
                    {
                        var method = orderingCount == 0 ? "OrderByDescending" : "ThenByDescending";
                        var parameter = param.Replace("desc", "").Trim();
                        query = query.OrderBy(parameter, method);
                    }
                    else
                    {
                        var method = orderingCount == 0 ? "OrderBy" : "ThenBy";
